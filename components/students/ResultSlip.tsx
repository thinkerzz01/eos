  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Student: <span className="font-medium text-slate-900 dark:text-slate-200">{studentName}</span> · {program} ({examSession})
          </p>
        </div>

        <div className="px-3 py-1 bg-purple-50 dark:bg-purple-950/50 border border-purple-200 dark:border-purple-800 rounded-xl text-xs font-medium text-[#5B47D6] dark:text-purple-300 self-start sm:self-auto">
          Cambridge Scale (A*-U)
        </div>
      </div>
            ...
            {grades.map((g, idx) => (
              <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                ...
                <td className="px-4 py-3.5 text-center">
                  <span className="font-heading font-medium text-slate-700 dark:text-slate-300 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    {g.internalAverage || '-'}
                  </span>
                </td>
                <td className="px-4 py-3.5 text-center">
                  {g.assessedGrade ? (
                    <span className="font-heading font-medium text-emerald-600 dark:text-emerald-400 px-3 py-1 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 rounded-lg text-sm">
                      {g.assessedGrade}
                    </span>
                  ) : (
                    <span className="text-xs font-medium text-slate-400 italic bg-slate-100 dark:bg-slate-800/60 px-2.5 py-1 rounded-lg">
                      Blank (Not yet tested)
                    </span>
                  )}
                </td>
                <td className="px-4 py-3.5 text-center">
                  <span className="font-heading font-medium text-[#5B47D6] dark:text-purple-300 px-3 py-1 bg-purple-50 dark:bg-purple-950/60 border border-purple-200 dark:border-purple-800 rounded-lg text-sm">
                    {g.targetGrade || 'A*'}
                  </span>
                </td>
              </tr>
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        <p>
          <strong className="text-slate-700 dark:text-slate-300">Policy Note:</strong> Target Grade defaults to <span className="font-medium text-[#5B47D6]">A*</span> at enrollment. Assessed Grade is assigned by the subject teacher after each formal test cycle. The monthly parent report references only the assessed-grade trend (up / same / down), keeping raw scores separate.
        </p>
      </div>
    </div>
  );
