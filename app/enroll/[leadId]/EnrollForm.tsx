...
// Enrollment form. The parent page (a server component) has already validated the
// lead behind this link and passes the booking data as `initial`, so every field
// arrives PRE-FILLED with exactly what the family submitted when they booked the
// demo - they only add what's new (gender, exam session, etc.).
...
export function EnrollForm({ leadId, initial }: { leadId: string; initial: EnrollInitial }) {
  ...
  const [program, setProgram] = useState(
    ALL_PROGRAMS.includes(initial.program as (typeof ALL_PROGRAMS)[number]) ? initial.program : ALL_PROGRAMS[0]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await submitEnrollment({
        leadId, studentName, parentName, phone, email, program, examSession, gender, city, address, turnstileToken,
      });
      if (!res.ok) {
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2 max-w-2xl mx-auto">
        ...
        <p className="text-sm text-[#6B7185] font-medium leading-relaxed">
          We&apos;ve filled in the details from your demo booking - please review, add the exam session, and complete enrollment. Our team will then set up the fee and class schedule.
        </p>
      </div>
