...
export function StudentsClient({
  ...
  // ASSIGN TEACHER & SUBJECTS MODAL (admin/manager: link an existing student to a
  // teacher via student_subjects, so they appear on that teacher's portal).
  const [assignStudent, setAssignStudent] = useState<Student | null>(null);
  const [assignRows, setAssignRows] = useState<{ subjectId: string; teacherId: string }[]>([]);
  ...
  const openAssign = (s: Student) => {
    setAssignStudent(s);
    setAssignError(null);
    setAssignRows([{ subjectId: '', teacherId: '' }]);
    // Prefill with the student's existing subject/teacher enrollments.
    listStudentEnrollments(s.id)
      .then((es) => {
        if (es.length) setAssignRows(es.map((e) => ({ subjectId: e.subjectId, teacherId: e.teacherId })));
      })
      .catch(() => { /* leave the single blank row */ });
  };
  const assignSubjectOptions = assignStudent
    ? subjects.filter((sub) => sub.program === assignStudent.program)
    : subjects;
  const updateAssignRow = (i: number, patch: Partial<{ subjectId: string; teacherId: string }>) =>
    setAssignRows((rows) => rows.map((r, idx) => (idx === i ? { ...r, ...patch } : r)));
  const addAssignRow = () => setAssignRows((rows) => [...rows, { subjectId: '', teacherId: '' }]);
  const removeAssignRow = (i: number) =>
    setAssignRows((rows) => (rows.length > 1 ? rows.filter((_, idx) => idx !== i) : rows));
  const handleSaveAssign = async () => {
    if (!assignStudent) return;
    setAssignError(null);
    const rows = assignRows.filter((r) => r.subjectId && r.teacherId);
    if (rows.length === 0) { setAssignError('Pick at least one subject and a teacher.'); return; }
    setAssignSaving(true);
    const res = await assignStudentSubjects({ studentId: assignStudent.id, enrollments: rows });
    setAssignSaving(false);
    if (res.ok) {
      setAssignStudent(null);
      router.refresh();
      showToast('Teacher & subjects assigned.', 'success');
    } else {
          ...
          <div className="bg-white dark:bg-slate-900 border border-[#EBEDF3] dark:border-slate-800 rounded-[18px] shadow-sm overflow-hidden">
            <div className="p-4 border-b border-[#F1F2F7] dark:border-slate-800">
              <div className="relative max-w-xs">
                ...
                <input
                  ...
                  className="w-full bg-[#F6F7FB] dark:bg-slate-800 border border-[#EBEDF3] dark:border-slate-700 rounded-xl pl-9 pr-3 py-2 text-sm font-medium text-slate-900 dark:text-slate-100 placeholder-[#9AA0B4] focus:outline-none focus:border-[#5B47D6]"
                />
              </div>
            </div>
            ...
            <div className="overflow-x-auto hidden md:block">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 text-[13px] text-[#6B7185]">
                    <th className="py-3.5 px-3 font-medium text-slate-900 dark:text-white">Student</th>
                    <th className="py-3.5 px-3 font-medium text-slate-900 dark:text-white">Program</th>
                    <th className="py-3.5 px-3 font-medium text-slate-900 dark:text-white">Subjects</th>
                    <th className="py-3.5 px-3 text-center font-medium text-slate-900 dark:text-white">Performance</th>
                ...
                <tbody className="divide-y divide-[#F1F2F7] dark:divide-slate-800">
                  {filteredStudents.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-10 text-center text-[#6B7185]">
                      ...
                      <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td className="py-3.5 px-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-medium text-[15px] text-slate-900 dark:text-slate-100">{s.name}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-3.5 px-3">
                          <div className="font-medium text-sm text-slate-900 dark:text-slate-100">{s.program}</div>
                          {s.grade && s.grade !== s.program && (
                        ...
                        <td className="py-3.5 px-3">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <div className="font-medium text-sm text-slate-900 dark:text-slate-100">{s.nextClassTime || 'No upcoming class'}</div>
                              {s.nextClassSubject && (
                                <div className="text-xs text-[#6B7185] font-medium mt-0.5">{s.nextClassSubject}</div>
                              )}
                              <Link
                                href="/schedule"
                                className="inline-flex items-center gap-1 mt-1.5 text-[11px] font-medium text-[#5B47D6] hover:underline"
                              >
                                View next class <ChevronRight className="w-3 h-3" />
                              </Link>
                            </div>
                          </div>
                        </td>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium text-[15px] text-slate-900 dark:text-slate-100 truncate">{s.name}</div>
                          <div className="text-xs text-[#6B7185]">{s.program}{s.grade && s.grade !== s.program ? ` · ${s.grade}` : ''}</div>
                        </div>
                      </div>
                      <span
                    ...
                    <div className="space-y-6 flex flex-col justify-between">
                      <div className="bg-white border border-[#EBEDF3] rounded-2xl p-5 space-y-4 shadow-sm">
                        ...
                        <div className="space-y-2.5 text-xs sm:text-sm">
                          ...
                          {isStaff && (<div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium flex items-center gap-1.5"><Tag className="w-3.5 h-3.5 text-slate-400" /> Roll Number</span><span className="font-medium text-slate-900">{profileModalStudent.rollNo}</span></div>)}
                          ...
                          {isStaff && (<div className="flex justify-between py-1.5"><span className="text-[#6B7185] font-medium flex items-center gap-1.5"><QrCode className="w-3.5 h-3.5 text-slate-400" /> Student ID</span><span className="font-mono font-medium text-slate-900">{profileModalStudent.stuId}</span></div>)}
                        </div>
                      </div>
                      ...
                      <div className="bg-white border border-[#EBEDF3] rounded-2xl p-5 space-y-4 shadow-sm">
                        ...
                        <div className="space-y-2.5 text-xs sm:text-sm">
                          ...
                          <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">School</span><span className="font-medium text-slate-900 text-right">{profileModalStudent.schoolName || profileModalStudent.onboardingExtra?.school || '-'}</span></div>
                          <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Gender</span><span className="font-medium text-slate-900 text-right capitalize">{profileModalStudent.gender || '-'}</span></div>
                          <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Subjects</span><span className="font-medium text-slate-900 text-right max-w-[60%]">{profileModalStudent.onboardingExtra?.subjects || '-'}</span></div>
                          <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Previous Result</span><span className="font-medium text-slate-900 text-right">{profileModalStudent.onboardingExtra?.previousResult || '-'}</span></div>
                          <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Preferred Time</span><span className="font-medium text-slate-900 text-right">{profileModalStudent.onboardingExtra?.preferredTime || '-'}</span></div>
                          {isStaff && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Student Email</span><span className="font-medium text-slate-900 text-right max-w-[60%] truncate" title={profileModalStudent.onboardingExtra?.studentEmail || ''}>{profileModalStudent.onboardingExtra?.studentEmail || '-'}</span></div>}
                          {isStaff && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Student Mobile</span><span className="font-medium text-slate-900 text-right">{profileModalStudent.onboardingExtra?.studentMobile || '-'}</span></div>}
                          {isStaff && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Parent Phone / WhatsApp</span><span className="font-medium text-slate-900 text-right">{profileModalStudent.onboardingExtra?.parentPhone || profileModalStudent.whatsapp || '-'}</span></div>}
                          {isStaff && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Parent Email</span><span className="font-medium text-slate-900 text-right max-w-[60%] truncate" title={profileModalStudent.onboardingExtra?.parentEmail || ''}>{profileModalStudent.onboardingExtra?.parentEmail || '-'}</span></div>}
                          ...
                          {isStaff && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-[#6B7185] font-medium">Address</span><span className="font-medium text-slate-900 text-right max-w-[60%] truncate" title={profileModalStudent.address || ''}>{profileModalStudent.address || '-'}</span></div>}
                          {profileModalStudent.onboardingExtra?.notes && <div className="flex justify-between py-1.5"><span className="text-[#6B7185] font-medium">Notes</span><span className="font-medium text-slate-900 text-right max-w-[60%]">{profileModalStudent.onboardingExtra.notes}</span></div>}
                        </div>
                      </div>
                    </div>
      ...
      {assignStudent && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-[#EBEDF3] dark:border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl space-y-4 my-6 text-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-[#6B7185] mt-0.5">
                  {assignStudent.name} · {assignStudent.program}. The student appears on each chosen teacher&apos;s portal.
                </p>
              </div>
              <button onClick={() => setAssignStudent(null)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
            ...
            <div className="space-y-3">
              {assignRows.map((r, i) => (
                <div key={i} className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl">
                  <div>
                    <label className="text-[11px] text-[#6B7185] font-medium block mb-1">Subject</label>
                    <select
                      value={r.subjectId}
                      onChange={(e) => updateAssignRow(i, { subjectId: e.target.value })}
                    ...
                    >
                      ...
                      {assignSubjectOptions.map((sub) => (<option key={sub.id} value={sub.id}>{labelWithCode(sub.name, sub.code)}</option>))}
                    </select>
                  </div>
                  <div>
                    ...
                    <select
                      value={r.teacherId}
                      onChange={(e) => updateAssignRow(i, { teacherId: e.target.value })}
                      className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-slate-900 dark:text-slate-100 font-medium"
                    >
                  ...
                  {assignRows.length > 1 && (
                    <button onClick={() => removeAssignRow(i)} className="text-xs font-medium text-rose-600 hover:underline text-left sm:col-span-2">
                      Remove
                    </button>
                  )}
                </div>
              ))}
              <button onClick={addAssignRow} className="text-xs font-medium text-[#5B47D6] hover:underline">+ Add another subject</button>
              {(subjects.length === 0 || teachers.length === 0) && (
                <p className="text-xs text-amber-600 font-medium">Add subjects and teachers first.</p>
              )}
              {assignError && (
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button onClick={() => setAssignStudent(null)} className="px-4 py-2.5 text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl">Cancel</button>
              <button onClick={handleSaveAssign} disabled={assignSaving} className="px-5 py-2.5 bg-[#5B47D6] hover:bg-[#4F3DC7] disabled:opacity-60 text-white text-xs font-medium rounded-xl shadow-sm">
