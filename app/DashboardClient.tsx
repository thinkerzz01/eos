    ...
    return (
      <PortalLayout title="" subtitle="" allowedRoles={['student']}>
        <div className="space-y-6 text-[#171A2B] dark:text-slate-100 max-w-full overflow-x-hidden pb-12 text-xs">
          ...
          {me && (me.feeStatus === 'Due' || me.feeStatus === 'In Grace') && (
            <Link
              href="/fees"
              className="flex items-center justify-between gap-3 rounded-[18px] border border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/10 px-4 py-3.5 hover:bg-rose-100/70 dark:hover:bg-rose-500/15 transition-colors"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="shrink-0 w-9 h-9 rounded-xl bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 grid place-items-center">
                  <Wallet className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <div className="font-medium text-sm text-slate-900 dark:text-slate-100">Fee {me.feeStatus === 'In Grace' ? 'in grace period' : 'due'}</div>
                  <div className="text-xs text-[#6B7185] dark:text-slate-400">View your voucher and payment details.</div>
                </div>
              </div>
              <span className="shrink-0 inline-flex items-center gap-1 text-xs font-medium text-rose-700 dark:text-rose-300">
