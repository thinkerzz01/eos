...
--     Seeds the Edexcel programs: 'Edexcel IGCSE' with real Int GCSE spec codes
--     (4MA1, 4PH1 …), 'Edexcel AS'/'Edexcel A2' with core subject names only
--     (set IAL codes in the Subjects manager). Idempotent; every org.
--     Full file: supabase/migrations/2026-09-11_edexcel_subjects.sql
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.subjects ADD COLUMN IF NOT EXISTS code TEXT;
DO $$
DECLARE
    v_org UUID; rec RECORD; prog TEXT; subj TEXT;
    ial_progs TEXT[] := ARRAY['Edexcel AS', 'Edexcel A2'];
    ial_subjects TEXT[] := ARRAY['Mathematics','Further Mathematics','Physics','Chemistry','Biology',
        'Economics','Business','Accounting','Computer Science','Information Technology',
        'English (First Language)','Literature in English','Geography','History','Psychology','Law'];
BEGIN
    FOR v_org IN SELECT id FROM public.orgs LOOP
        FOR rec IN SELECT * FROM (VALUES
