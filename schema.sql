...
CREATE TABLE IF NOT EXISTS public.students (
    ...
    date_of_birth DATE, -- collected during onboarding
    onboarding_data JSONB, -- extra onboarding answers (school, CNIC, emergency contact, etc.)
    onboarding_completed_at TIMESTAMPTZ, -- set when the student submits the onboarding form
...
-- ----------------------------------------------------------------------------
-- Student onboarding form (public, keyed by student id). The academy sends the
...
BEGIN
    ...
    v_gender := CASE WHEN lower(COALESCE(p_gender, '')) IN ('male', 'female', 'other')
                     THEN lower(p_gender) ELSE NULL END;
